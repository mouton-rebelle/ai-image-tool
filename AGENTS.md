...
- **Metadata**: ComfyUI writes its API prompt graph into the container metadata (`prompt`, or a `{"prompt": ..., "workflow": ...}` envelope in the `comment` tag), and Civitai preserves it through its re-encode. `comfy_workflow.go` walks the graph: it resolves the prompt through the sampler's positive/negative inputs when there is a text encoder, then through a guider chain (`SamplerCustomAdvanced` → `BasicGuider` → conditioning), and falls back to nodes carrying the prompt as a plain widget (MiniMax H3, Wan, LTXV) or as a generic string widget (`PrimitiveString*`, whose text ComfyUI builds with subgraphs flatten into the graph under ids like `22:11`). Bare `NaN`/`Infinity` tokens ComfyUI writes in some nodes (`is_changed`) are tolerated, and an unparsable JSON blob is dropped rather than stored as the prompt. Checkpoints found this way are registered as local models (`hash = "local:<name>"`), so they never hit the Civitai API but still appear in the model filter.
